...
def load_nids_components():
    ...
    scaler_path = os.path.join(BASE_DIR, 'nids_scaler.pkl')
...
with tab2:
    ...
    with col1:
        ...
        for i, feat in enumerate(feature_names[:mid_point]):
            user_inputs[feat] = st.number_input(f"{feat}", value=0.0, key=f"f_{i}")

    with col2:
        st.subheader("Packet Metrics")
        for i, feat in enumerate(feature_names[mid_point:]):
            user_inputs[feat] = st.number_input(f"{feat}", value=0.0, key=f"f_{i+100}")
